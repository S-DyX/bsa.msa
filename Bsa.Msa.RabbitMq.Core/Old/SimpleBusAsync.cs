...
//			Action<Func<IChannel>> configureAction = getChannel =>
//			{
//				var dictionary = GetArguments(messageHandlerSettings);
//				getChannel().QueueDeclareAsync(queueName, true, false, messageHandlerSettings.AutoDelete, dictionary).Wait();
//				getChannel().BasicQosAsync(0, messageHandlerSettings.PrefetchCount, false).Wait();
//			};
...
//		private void Consume<TMessage>(string queueName, Action<TMessage> action, Action<Func<IChannel>> configure)
//		{
...
//					try
//					{
//						getChannel.Invoke().QueuePurgeAsync(queueName).Wait();
//					}
//					catch (Exception ex)
//					{
...
//		private Action<Func<IChannel>> GetExchangeConfigure<TMessage>(string queueName, IMessageHandlerSettings settings)
//		{
...
//			Action<Func<IChannel>> action = getModel =>
//			{
//				var exchangeName = _busNaming.GetExchangeName<TMessage>();
//				getModel().ExchangeDeclareAsync(exchangeName, type, true).Wait();
//				getModel().QueueDeclareAsync(queueName, true, false, _messageHandlerSettings.AutoDelete, dictionary).Wait();
//				getModel().QueueBindAsync(queueName, exchangeName, routingKey ?? String.Empty).Wait();
//				getModel().BasicQosAsync(0, _messageHandlerSettings?.PrefetchCount ?? (ushort)5, false).Wait();

//			};
//			return action;
//		}
//		private Action<Func<IChannel>> GetRespondeConfigure(string queueName)
//		{
//			Action<Func<IChannel>> action = getModel =>
//			{
//				getModel().QueueDeclareAsync(queueName, false, false, true, null).Wait();
//				getModel().BasicQosAsync(0, _messageHandlerSettings != null ? _messageHandlerSettings.PrefetchCount : (ushort)5, false).Wait();
//			};
//			return action;
//		}
...
//			{
//				getChannel().ExchangeDeleteAsync(exchangeName, true).Wait();
//			});
//		}
...
//		private bool TryGetMessage<TMessage>(string queueName, Func<IChannel> getChannel, out TMessage data)
//		{
//			data = default(TMessage);
//			var isEmpty = true;
//			var getResult = getChannel().BasicGetAsync(queueName, false).Result;
//			if (getResult == null)
//				return false;
...
//			// ... process the message
//			getChannel().BasicAckAsync(getResult.DeliveryTag, false);
//			return !isEmpty;
//		}

//		private const int _sleepTime = 500;
//		private int _iteration = 1;
//		private void QueueingBasicConsumer<TMessage>(string queueName, Action<TMessage> action, Func<IChannel> getChannel, Action<Func<IChannel>> configure)
//		{
...
//				_consumer = new AsyncEventingBasicConsumer(getChannel.Invoke());
//				getChannel().BasicConsumeAsync(queueName, false, _consumer).Wait();
//				_logger?.Info($"Start BasicConsume {_queueName};{_messageHandlerSettings.DegreeOfParallelism}");
//				if (!_messageHandlerSettings.TurnOffInternalQueue)
//					_consumer.ReceivedAsync += consumerOnReceived(queueName, action, getChannel);
//				else
//				{
//					_consumer.ReceivedAsync += consumerOnReceivedOnlyRmq(queueName, action, getChannel);
//				}
//				_logger?.Info($"End BasicConsume {_queueName};{_messageHandlerSettings.DegreeOfParallelism}");
//			}
//		}

//		private void ProcessFromLocalBus<TMessage>(string queueName, Action<TMessage> action, Func<IChannel> getChannel)
//		{
//			try
//			{
//				_logger?.Debug($"Try get value from local bus ProcessFromLocalBus {queueName}");
//				var items = _internalBus.Get(queueName);

//				_logger?.Debug($"Invoke LocalBus {queueName}; Count:{items.Count}");
//				if (items.FastAny())
//				{
...
//							try
//							{
//								_logger?.Debug($"Process InternalBus {queueName}; Id:{item.Id}");
//								ProcessMessageInternalBus(queueName, action, getChannel, item, default(TMessage));
//								_logger?.Debug($"Processed InternalBus {queueName}; Id:{item.Id}");
//							}
//							catch (Exception e)
//							{
...
//		private byte[] ProcessMessageInternalBus<TMessage>(string queueName, Action<TMessage> action, Func<IChannel> getChannel, InternalBusItem item, TMessage message)
//		{
...
//				}
//				_logger?.Debug($"Invoke task messageId:{item.Id} {queueName}");
//				action.Invoke(message);
//				_internalBus.Ack(item.Id);
//				_logger?.Debug($"Ack messageId:{item.Id} {queueName}");
//			}
...
//		private AsyncEventHandler<BasicDeliverEventArgs> consumerOnReceived<TMessage>(string queueName, Action<TMessage> action, Func<IChannel> getChannel)
//		{
//			return (ch, e) =>
//			{
//				_logger?.Debug($"Receive message {_queueName};");
...
//					{
//						getChannel().BasicNackAsync(e.DeliveryTag, false, true);
						
//						_logger?.Debug($"Wait free thread: To many threads Sleep {_queueName};{tasks.Count}>{_messageHandlerSettings.DegreeOfParallelism}");
...
//						getChannel().BasicAckAsync(e.DeliveryTag, false);
...
//						channel.BasicRejectAsync(deliveryTag.Value, true);
//					}
...
//		private void UnsubscribeFromQueue<TMessage>(string queueName, Action<TMessage> action, Func<IChannel> getChannel)
//		{
...
//		private void ProcessException<TMessage>(string queueName, Func<IChannel> getChannel, Exception ex, string messageAsString,
...
//		private AsyncEventHandler<BasicDeliverEventArgs> consumerOnReceivedOnlyRmq<TMessage>(string queueName, Action<TMessage> action, Func<IChannel> getChannel)
//		{
//			return (ch, e) =>
//			{
//				var tasks = _tasks.Where(x => x.IsActive).ToList();
...
//		private void ProcessSingleRmqMessage<TMessage>(string queueName, Action<TMessage> action, Func<IChannel> getChannel, BasicDeliverEventArgs e)
//		{
//			ulong? deliveryTag = null;
//			try
//			{
...
//		private void SendErrorMessage(IChannel channel, string queueName, byte[] body, Exception ex)
//		{
...
//		}
//		private void Send(IChannel channel, string queue, byte[] body, IDictionary<string, object> headers)
//		{
//			try
//			{

//				var dictionary = GetArguments(_messageHandlerSettings);
//				channel.QueueDeclareAsync(queue, true, false, false, dictionary);
//				var props = channel.CreateBasic();
//				props.DeliveryMode = 2;
//				props.Headers = headers;
//				channel.BasicPublishAsync("", queue, props, body);
//			}
//			catch (Exception ex)
//			{
//				_logger?.Error(ex.Message, ex);
//			}
//		}
//		private void SendErrorMessage(IChannel channel, string queueName, string message, Exception ex)
//		{
//			var errorQueue = queueName + ".Error";
...
//		private void Send(IChannel channel, string queue, string message, IDictionary<string, object> headers)
//		{
//			try
//			{

//				var dictionary = GetArguments(_messageHandlerSettings);
//				channel.QueueDeclareAsync(queue, true, false, false, dictionary).Wait();
...
//			{
//				getChannel().QueueDeclareAsync(queue, true, false, false, null);
//			}, true);
//			_simpleConnection.Execute(getChannel =>
//			{
...
//				props.DeliveryMode = 2;
//				getChannel().BasicPublishAsync("", queue, props, body);
//			});
...
//				getChannel().ExchangeDeclareAsync(exchangeName, type, true);
//			});
...
//				var props = getChannel().CreateBasicProperties();
//				props.DeliveryMode = 2;
//				getChannel().BasicPublishAsync(exchangeName, routingKey ?? String.Empty, props, body);
//			});
//		}
...
//			{
//				getChannel().ExchangeDeleteAsync(exchangeName, true).Wait();
//				getChannel().QueueDeleteAsync(queue).Wait();
//			});
//		}
...
//				getChannel().BasicPublishAsync(exchangeName, string.Empty, props, body);
//			});
//		}
...
//				getChannel().BasicPublishAsync("", props.ReplyTo, replyProps, b);
